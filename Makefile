...
PROTOS   := proto/datatransform/v1/datatransform.proto proto/patientdata/v1/patientdata.proto
BIN      := bin/data-transform-service

.PHONY: proto build run test docker compose-up compose-down tidy tools

## Gera o código Go. Mensagens (datatransform + patientdata) e o service gRPC
## apenas do datatransform (não somos cliente do patient-data no Modelo B).
proto:
	protoc -I proto \
